      ...
      <div class="mods-list" v-if="modrinthSearchResult.hits.length > 0">
        <div
          ...
          <div class="actions" @click.stop>
            <button
              class="heart"
              :class="{ 'heart-favorited': isFavorited('modrinth', 'pack', pack.project_id) }"
              @click.stop="toggleFavorite('modrinth', 'pack', pack.project_id)">
              <AppIcon
                :name="isFavorited('modrinth', 'pack', pack.project_id) ? 'heart' : 'heart-outline'"
                :size="14"></AppIcon>
            </button>
            <button class="link">
              <AppIcon
...
function filterByFavorites(result: ModrinthSearchedProjects): ModrinthSearchedProjects {
  ...
  return {
    ...result,
    hits: result.hits.filter((hit) => isFavorited("modrinth", "pack", hit.project_id)),
  };
}
  ...
  },
  {
    ...
    display: (option) =>
      option === "favorites"
        ...
        : (t(`overlays.content.packs.modrinth.${option as string}`) ?? option),
