...
<template>
  <div class="minecraft-choose">
    <div class="filter">
      <p>{{ t("overlays.dialogs.minecraftChoose.versionCategory") }}</p>
      <!-- TODO: Special version support -->
      <base-select
        :options="['releases', 'snapshot', 'old']"
        :display-name="[
          t('overlays.dialogs.minecraftChoose.releases'),
          t('overlays.dialogs.minecraftChoose.snapshot'),
          t('overlays.dialogs.minecraftChoose.old'),
        ]"
...
function parseTime(time: string) {
  ...
  return t("game.time.yearMonthDay", {
...
function clickAbout(version: string) {
  if (locale.value.startsWith("zh")) {
    ...
    openUrl(`https://minecraft.wiki/w/Java_Edition_${version}`);
  }
}
</script>
